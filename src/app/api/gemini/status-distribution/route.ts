...
import { analyzeStatusDistribution } from "@/server/queries/gemini";
import type {
...
function getCacheKey(input: StatusDistributionInput): string {
...
export async function POST(
  ...
  try {
    ...
    const input = body as StatusDistributionInput;
    ...
    if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
      console.log("Returning cached status distribution AI");
      return NextResponse.json({
        success: true,
    ...
    console.log(`Generating AI insights for status distribution`);

    // Generate AI analytics
    const analytics = await analyzeStatusDistribution(input);
    ...
    console.log("Status distribution AI generated successfully");
    ...
    console.error("Error generating status distribution AI:", error);
